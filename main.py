...
def main():
    # 1. Считываем mails.txt (логин@gmail.com   пароль  доп.почта   ГЕО UserAgent   cookies [{JSON}]))
    ...
    for mail in mails:
        login, password, reserv_mail, geo, ua, coockies = mail.split('\t')

        database.save_email_to_db(login, password, reserv_mail, geo, ua, coockies)
        logins.append(login)
    ...
    print(f'[{len(accounts)}] Accounts запущено')
    ...
    for login, password, reserv_mail, user_agent in accounts:
        ...
        try:
            ...
            with open('valid_logins.txt', 'a') as file:
                file.write(login + '\n')
            ...
            # Выгружаем куки из браузера
            # functions.cookies_from_browser (driver, login)
