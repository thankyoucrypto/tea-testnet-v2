...
# ФУНКЦИЯ ДЛЯ ВЫГРУЗКИ COOKIE В ФАЙЛ
def cookies_from_browser(driver, login):
    try:
        cookies_dir = 'cookies'
        if not os.path.exists(cookies_dir):
            os.makedirs(cookies_dir)

        cookies_path = os.path.join(cookies_dir, f'{login}.dat')
        cookies = driver.get_cookies()
        if cookies:  # Проверка на наличие куки
            with open(cookies_path, 'wb') as f:
                pickle.dump(cookies, f)
            print(f'Выгрузили cookie аккаунта в {cookies_path}')
        else:
            print('Нет куки для выгрузки.')
        time.sleep(2)
        return True
    except Exception as e:
        print(f'Ошибка при выгрузке куки в файл: {e}')
        return False


# ФУНКЦИЯ ДЛЯ ДОБАВЛЕНИЯ КУКИ В БРАУЗЕР
def cookies_to_browser(driver, cookies):
    # Преобразование JSON-строки в список словарей
    cookies_list = json.loads(cookies)
    print(cookies_list)
    print('------')

    # Добавление каждого куки в браузер
...
def change_ip(proxy_links):
    while True:
        ...
        try:
            response = requests.get(f'{link}&format=json', timeout=60)
            if response.status_code == 200:
                ...
                if new_ip:
                    ...
                    print('Failed to extract new IP from response. Try again')
            else:
                print(f'Failed to change IP: {response.text}. Try again')
        except requests.exceptions.ReadTimeout:
            print('Request timed out. Retrying...')
        except requests.exceptions.RequestException as e:
            print(f'An error occurred: {e}')

        time.sleep(10)  # Добавляем задержку перед следующей попыткой
