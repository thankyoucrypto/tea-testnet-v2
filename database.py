...
def save_email_to_db(login, password, reserv_mail, geo, user_agent, coockies):
    conn = sqlite3.connect('accounts.db')
    c = conn.cursor()
    ...
    if existing_account:
        ...
        c.execute('INSERT INTO accounts (login, pass, reserv_mail, geo, user_agent, coockies) VALUES (?, ?, ?, ?, ?, ?)', (login, password, reserv_mail, geo, user_agent, coockies))
        conn.commit()
...
def get_all_accounts():
    conn = sqlite3.connect('accounts.db')
    c = conn.cursor()
    c.execute('SELECT login, pass, reserv_mail, user_agent FROM accounts ')
    accounts = c.fetchall()
    conn.close()
    return accounts
...
def get_accounts_from_file(logins):
    ...
    query = f'SELECT login, pass, reserv_mail, geo, user_agent, coockies FROM accounts WHERE login IN ({placeholders})'
